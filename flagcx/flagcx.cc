...
flagcxResult_t flagcxHandleInit(flagcxHandlerGroup_t *handler) {
  ...
  printf("%s:%d deviceAdaptor name %s", __func__, __LINE__, deviceAdaptor->name);
...
flagcxResult_t flagcxCommInitRank(flagcxComm_t *comm, int nranks,
  ...
  for (int i = 0; i < nranks; ++i) {
    INFO(FLAGCX_INIT, "rank=%d, clusterIdData[%d]=%d, clusterInterRanks[%d]=%d", i, i, clusterIdData[i], clusterIdData[i], clusterInterRanks[clusterIdData[i]]);
...
flagcxResult_t flagcxAllGather(const void *sendbuff, void *recvbuff,
  ...
  if (is_homo_comm(comm)) {
    ...
    if (use_host_comm()) {
      ...
      if (comm->support_multi_nic < 0) {
        INFO(FLAGCX_COLL, "%s:%d ############ not support_multi_nic", __func__, __LINE__);
